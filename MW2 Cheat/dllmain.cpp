...
static bool is_valid_address(type* ptr)
{
	if (ptr < (type*)0xFFFFFF || ptr >(type*)0x7FFFFFFF0000)
		return false;
...
__declspec(dllexport)HRESULT present_hk(IDXGISwapChain3* pSwapChain, UINT SyncInterval, UINT Flags)
{
	...
	if (!is_valid_address(d3d12test::d3d12CommandQueue2))
		d3d12test::d3d12CommandQueue2 = *(ID3D12CommandQueue**)(reinterpret_cast<uint64_t>(pSwapChain) + 0x118); // windows 10

	if (!is_valid_address(d3d12test::d3d12CommandQueue2))
		d3d12test::d3d12CommandQueue2 = *(ID3D12CommandQueue**)(reinterpret_cast<uint64_t>(pSwapChain) + 0x168); // windows 11
